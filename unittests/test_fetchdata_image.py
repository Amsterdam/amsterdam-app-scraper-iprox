...
from unittests.mock_functions import mocked_requests_get, mocked_requests_post, MockThread
...
class Unittests(unittest.TestCase):
    ...
    def test_fetch_image(_mocked_requests_get, _mocked_requests_post, _MockThread):
        """ Test fetch image from url """
        ...
        with patch.object(Logger, 'error', return_value=None) as mock_method:
            image_fetcher.run()
            assert _mocked_requests_get.call_count == 1
            assert _mocked_requests_post.call_count == 2
